...
def train_D(D, real_data, real_labels, generated_fake_data, true_labels, false_labels, D_optimizer, D_loss_fn):
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(enumerate(train_loader), total=len(train_loader), unit="batch", ascii=True, desc="Epoch {}".format(epoch)) as tepoch:
            for bid, (x, y) in tepoch:
                ...
                D.train()
                G.eval()
                d_loss = train_D(D, x, y, G.generate(batch_size, y).detach(), true_labels, false_labels, D_optimizer, D_loss_fn)
                epoch_d_loss += d_loss
        ...
        G.eval()
        epoch_image = G.generate(200, torch.nn.functional.one_hot(torch.randint(0, 10, (200, ))).type(torch.FloatTensor).to(device)).cpu().detach().view(-1,1,28,28)
